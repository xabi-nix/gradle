...
public class DependencyClassPathNotationConverter implements NotationConverter<DependencyFactory.ClassPathNotation, SelfResolvingDependency> {
    ...
    private SelfResolvingDependency maybeCreateUnderLock(DependencyFactory.ClassPathNotation notation) {
        ...
        if (dependency == null) {
            ...
            if (notation.equals(DependencyFactory.ClassPathNotation.GRADLE_API)) {
    ...
    private FileCollectionInternal apiJar(Collection<File> classpath) {
        return new FileCollectionAdapter(new GradleImplDepsRelocatedJar("gradleApi()", classpath, getGradleApiFarJarOutputFile()));
    }
