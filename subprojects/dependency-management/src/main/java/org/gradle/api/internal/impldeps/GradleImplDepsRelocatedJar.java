...
public class GradleImplDepsRelocatedJar implements MinimalFileSet {
    ...
    public Set<File> getFiles() {
        if (!outputJar.exists()) {
            ...
            System.out.println("Starting fat jar creation: " + outputJar.getAbsolutePath());
            long start = System.currentTimeMillis();
            GradleImplDepsRelocatedJarCreator.create(outputJar, files);
            long stop = System.currentTimeMillis();
            System.out.println("Finished fat jar creation, " + (stop - start) / 1000 + "s");
        } else {
            System.out.println("Fat jar exists: " + outputJar.getAbsolutePath());
        }
